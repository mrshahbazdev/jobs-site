...
class JobController extends Controller
{
    public function index(Request $request)
    {
        ...
        $landingGroups = LandingGroup::active()->ordered()->with(['links' => fn ($q) => $q->active()->ordered()])->get();
        $homeBlocks = HomeBlock::active()->where('page_slug', 'home')->ordered()->get();
    ...
    {
        $job = JobListing::where('slug', $slug)->with(['category', 'city'])->first();
        abort_if(! $job, 410);
