    ...
    <main class="mx-auto flex flex-col lg:flex-row w-full max-w-7xl grow gap-8 px-4 py-8 lg:px-10">
        <aside class="w-full lg:w-64 shrink-0 flex-col gap-4 flex">
            <div id="filterSidebar" class="flex flex-col gap-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                ...
                <h3 class="text-lg font-bold">Other Cities</h3>
                <div class="flex flex-col gap-2">
                    @foreach($cities as $c)
                        <a href="{{ url('/cities/'.$c->slug) }}" class="text-sm {{ $c->id == $city->id ? 'text-primary font-bold' : 'text-slate-600 dark:text-slate-400' }} hover:text-primary">
                            {{ $c->name }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="flex flex-1 flex-col gap-8">
            <div class="flex flex-col gap-3">
                <h1 class="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Jobs in {{ $city->name }}</h1>
                <p class="text-base text-slate-500 dark:text-slate-400 leading-relaxed">Browse all current openings in {{ $city->name }} and nearby areas.</p>
            </div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    @forelse($jobs as $job)
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ $job->category->name }}</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                            <p class="text-slate-500">No jobs found in {{ $city->name }} yet. Check back soon!</p>
                        </div>
                    @endforelse
                </div>
