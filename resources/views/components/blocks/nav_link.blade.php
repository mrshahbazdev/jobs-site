...
@if($isButton)
    <a href="{{ $url }}" class="inline-flex items-center justify-center rounded-lg {{ $block->icon === 'chat' ? 'bg-[#25D366]/10 text-[#25D366] border-[#25D366]/20' : 'bg-primary/10 text-primary border-primary/20' }} px-4 py-2.5 text-sm font-black hover:opacity-80 transition-colors border">
        @if($block->icon)
            @if($block->icon === 'chat')
...
@else
    <a href="{{ $url }}" class="inline-flex items-center justify-center rounded-lg px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
        @if($block->icon)
            <span class="material-symbols-outlined mr-2">{{ $block->icon }}</span>
        @endif
        {{ $block->title }}
