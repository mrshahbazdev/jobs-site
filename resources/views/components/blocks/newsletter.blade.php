    ...
    <div class="mx-auto max-w-7xl">
        <div class="rounded-3xl bg-primary p-8 md:p-16 text-center text-white relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col items-center gap-6">
                ...
                <p class="max-w-2xl text-lg text-primary-100">{{ $block->sub_text ?? 'Subscribe to our daily newsletter or join our WhatsApp community to get the latest job alerts delivered directly to you.' }}</p>
                ...
                <form action="{{ route('subscribe') }}" method="POST" class="flex w-full max-w-md flex-col gap-3 sm:flex-row">
                    @csrf
                    <input name="email_or_phone" required class="flex-1 rounded-xl border-0 px-4 py-4 text-slate-900 placeholder:text-slate-500 focus:ring-2 focus:ring-white" placeholder="Email or WhatsApp number" type="text"/>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <span class="text-sm font-bold uppercase tracking-wider text-white/60">OR</span>
                    <span class="h-px w-8 bg-white/30"></span>
                </div>

                <a href="https://chat.whatsapp.com/your-group-link" class="flex items-center gap-3 rounded-xl bg-[#25D366] px-8 py-4 font-black text-white hover:bg-[#25D366]/90 transition-colors shadow-lg">
